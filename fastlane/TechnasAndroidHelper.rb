...
module TechnasAndroidHelper
  def process_json_key(env_var_name, output_file = 'play_config.json')
    ...
    UI.user_error!("Environment variable #{env_var_name} is not set!") if content.to_s.empty?

    3.times do |i|
      begin
        JSON.parse(content)
        UI.message("Valid JSON found after #{i} decode(s)")
        ...
        begin
          ...
          UI.user_error!("Failed to decode #{env_var_name}: #{e.message}")
        end
      end
    end

    UI.user_error!("Could not extract valid JSON from #{env_var_name} after multiple decodes")
  end
  ...
  def technas_distribute_apk(apk_path:)
    UI.user_error!("APK path not provided") unless apk_path
    UI.user_error!("UPLOADS_SECRET_AUTH_TOKEN not set") unless ENV['UPLOADS_SECRET_AUTH_TOKEN']
